...
%     end
%     allocationVector=type;
% end
% dataSet = [dataSet(:,1:end) num2cell(type)];
...
% clear lib M_RP M_TP ID_unici;
% clear idx_TP idx_RP;
% clear real_lat real_long;
% clear iTP iRP;
% clear RP_operatore_ID RP_NCPI_ID;
% clear TP_operatore_ID TP_NPCI_ID;
%
...
if PCA == 0
    ...
    M_RP = Features(allocationVector==1,:);
    M_TP = Features(allocationVector==2,:);
    idx1=ones(size(M_TP));
    idx2=ones(size(M_RP));
end

M_TP(isnan(M_TP))=missRefValue;
M_RP(isnan(M_RP))=missRefValue;
dummyTPs=~any(idx1,2);
nDummyTPs=nnz(dummyTPs);
dummyRPs=~any(idx2,2);
...
%idx1=zeros(size(M_TP,1),size(M_TP,2));
%idx2=zeros(size(M_RP,1),size(M_RP,2));

%for i=1:size(M_TP,1)
%idx1(i,find(M_TP(i,:)~=missRefValue))=1;%
%end
%for j=1:size(M_RP,1)
% idx2(j,find(M_RP(j,:)~=missRefValue))=1;
%end
