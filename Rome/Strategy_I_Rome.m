% This script implements Strategy I for the Rome dataset as described in the following research paper:
...
for npca=1:length(PCA_var_percVect)
    ...
    save(['Strategy_I_Rome_RF_param_' paramStr '_PCA_Perc_' num2str(PCA_var_perc) '_' num2str(nRuns) '_runs.mat']);
    ...
    OnTime_I_Rome(npca)=(Positioning_time)/nRuns;
    OffTime_I_Rome(npca)= PCA_time/nRuns;
    errAverage_I_Rome{npca}=err_mean;
    errVariance_I_Rome{npca}=err_variance;
    errStd_I_Rome{npca}=err_std;
end

save PCA_Sweep_Strategy_I_Rome errAverage_I_Rome errVariance_I_Rome errStd_I_Rome OffTime_I_Rome OnTime_I_Rome nRuns PCA_var_percVect results
