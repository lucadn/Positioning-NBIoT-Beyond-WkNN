%This script implements Strategy IV for the Rome dataset as described in the following research paper:
...
%dataFileName=sprintf('Campaign_data_NBIoT_1_2_3_4_5_6_interp.mat');
...
for npca=1:length(PCA_var_percVect)
    PCA_var_perc=PCA_var_percVect(npca);% percentuale di varianza totale da preservare
    time=tic;
    ...
    for ii = 1:length(NclustVect)
        for jj = 1:length(Mvect)
            ...
            for r = 1:nRuns
                [centers{r},trainingSet{r}, testSet{r}, models{r}, XTest{r},trainingSet_UTM{r}] = FCM_ranforest_alloc_vec(dataSet, TrainingPerc, TP_RP_allocation_Rome(:,r), NclustVect(ii), Mvect(jj), RF_param-1, bounds);
            end
            ...
            %pos_error=zeros(nRuns,kMax);
            for k = 1:nRuns
                subSetIndex{k} = getSubsetIDX(trainingSet_UTM{k}, coarseCoordsTest{k}, R_fp); % maschera per isolare i sottoinsiemi di trainingSet
            end
            ...
            for k = 1:nRuns
                ...
                %[tp_errors{k},average_error_tmp(k,:),Delta{1,k},percLocated(1,k),TPs_located, best_k, W]=Wei_Cov_strategy_FCM(1,trainingSet{k},testSet{k},subSetIndex{k}, 0, RF_param, -1,kMax,allocationVector);
            ...
            save(['Strategy_IV_Rome_RF_param_' paramStr '_numClusters_' num2str(NclustVect(ii)) '_M_' num2str(Mvect(jj)) '_PCA_Perc_' num2str(PCA_var_perc) '_' num2str(nRuns) '_runs.mat']);
            ...
            OnTime_IV_Rome(ii,npca)=(Online_time)/nRuns;
            OffTime_IV_Rome(ii,npca)=(Offline_time)/nRuns;
            errAverage_IV_Rome{ii,npca}=err_mean;
            errVariance_IV_Rome{ii,npca}=err_variance;
            errStd_IV_Rome{ii,npca}=err_std;

        end
    end
end
save K_m_Sweep_Strategy_IV_Rome errAverage_IV_Rome errVariance_IV_Rome errStd_IV_Rome OffTime_IV_Rome OnTime_IV_Rome nRuns Mvect NclustVect results
