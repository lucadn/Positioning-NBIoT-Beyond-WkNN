%This script implements Strategy V for the Oslo dataset as described in the following research paper:
...
for npca = 1:length(PCA_values)
    ...
    for val_idx = 1:length(Validation_values)
        ...
        for vf_idx = 1:length(vf_values)
            ...
            % Reset M to its original version for this configuration.
            %M = M_orig;
            ...
            for r = 1:nRuns
                ...
                trainIndexes = find(training(c));  % 70% of the data
                testIndexes  = find(test(c));        % 30% of the data
                ...
                if currentValidation > 0
                    numValidation = round(length(trainIndexes) * currentValidation);
                    valIndexes = trainIndexes(randperm(length(trainIndexes), numValidation));
                else
                    valIndexes = [];
                end

                % Split the feature data.
                dataPredTrain = M(trainIndexes, :);
                dataPredTest  = M(testIndexes, :);
                if ~isempty(valIndexes)
                    dataPredValidation = M(valIndexes, :);
                end

                % Get the corresponding target coordinates (normalized).
                YTrain = Points_coords_n(trainIndexes, :);
                YTest  = Points_coords_n(testIndexes, :);
                if ~isempty(valIndexes)
                    YValidation = Points_coords_n(valIndexes, :);
                end

                % Also store the original coordinates for error analysis.
                testPointCoordinates = Points_coords(testIndexes, :);
                trainingPointCoordinates = Points_coords(trainIndexes, :);
            ...
            filename = sprintf('Strategy_V_Oslo_RF_param_%s_PCA_%d_Val_%.2f_vf_%d.mat', paramStr, PCA_var_perc, currentValidation, currentVF);
            ...
            OffTimePCA_V_Oslo(npca) = PCA_time/nRuns;
            OffTime_V_Oslo(npca) = Offline_time/nRuns;
            OnTime_V_Oslo(npca) = Online_time/nRuns;
            errAverage_V_Oslo{npca}=err_mean;
            errVariance_V_Oslo{npca}=err_variance;
            errStd_V_Oslo{npca}=err_std;
            results = [results; PCA_var_perc, err_mean, err_std, err_variance, err_mean_std, PCA_time Training_time Offline_time Online_time];
        end
    end
end

save PCA_Sweep_Strategy_V_Oslo errAverage_V_Oslo errVariance_V_Oslo errStd_V_Oslo OffTimePCA_V_Oslo OffTime_V_Oslo OnTime_V_Oslo nRuns PCA_vector results
