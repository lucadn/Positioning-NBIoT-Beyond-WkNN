%This script implements Strategy II for the Oslo dataset as described in the following research paper:
...
for nc=1:length(NclustVect)
    ...
    for npca=1:length(PCA_var_percVect)
        ...
        for r = 1:nRuns                     
            ...
            %for c = 1:Nclust    
                %RP_in_clust_coords{c,r} = RP_coords_tmp(clusters(:,r)==c,:); % coordinates of RPs in cluster c for run r
                %RP_in_clust{c,r} = RP(clusters(:,r)==c,r); % indexes of RPs in cluster c for run r;
            %end
        ...
        save(['Strategy_II_Oslo_RF_param_' paramStr '_numClusters_' num2str(Nclust) '_PCA_Perc_' num2str(PCA_var_perc) '_' num2str(nRuns) '_runs.mat']);
        ...
        OnTime_II_Oslo(nc,npca)=(Positioning_time+Testing_time)/nRuns;
        OffTime_II_Oslo(nc,npca)=(PCA_time+Clustering_time+Training_time)/nRuns;
        OffTimePCA_II_Oslo(nc,npca)=(PCA_time)/nRuns;
        errAverage_II_Oslo{nc,npca}=err_mean;
        errVariance_II_Oslo{nc,npca}=err_variance;
        errStd_II_Oslo{nc,npca}=err_std;
    end
end

save K_PCA_Sweep_Strategy_II_Oslo errAverage_II_Oslo errVariance_II_Oslo errStd_II_Oslo OffTimePCA_II_Oslo OffTime_II_Oslo OnTime_II_Oslo nRuns PCA_var_percVect NclustVect results
